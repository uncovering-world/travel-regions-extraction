...
Версия: 0.1.1-draft. Дата: 2026-09-12. Статус: экспериментальная спецификация, не утверждённая мировая классификация.
...
Территориально ограниченное разрешение на посещение рассматривается как кандидат split. Билет в музей, доступ в здание, охраняемый участок, военную базу или локальный заповедник по умолчанию хранится как spatial access overlay. Общая граница между такими объектами и самостоятельной restricted destination пока не определена. Размер площади, посещаемость и длительность разрешения не объявляются скрытыми порогами. Если исход зависит именно от этой границы, результат `model_unresolved`.
